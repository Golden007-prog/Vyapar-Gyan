...
const logger = createLogger({ handler: 'bedrock-action-group-executor' });
 ...
 */
async function handleListProductsByCategory(
  event: BedrockActionGroupEvent,
  catalogRepo: CatalogRepository
): Promise<BedrockActionGroupResponse> {
  ...
  const responseBody = response.response.responseBody?.['application/json'];
  if (responseBody?.body) {
    responseBody.body = JSON.stringify({
